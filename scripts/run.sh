...
usageState1["setup"]="SETUP THE ENVIRONMENT"
usageState1["gazebo-classic-PILS"]="DEPLOY GAZEBO PILS CONTAINER"
usageState1["gazebo-classic-airsim-PILS"]="DEPLOY GAZEBO-AIRSIM PILS CONTAINER"
usageState1["px4"]="DEPSaticefiedLOY PX4-AUTOPILOT CONTAINER"
...
if [ "$1x" == "setupx" ]; then
    ...
    usageState2["sim"]="SETUP THE SIMULATION ENVIRONMENT"
    usageState2["onboard"]="SETUP THE ONBOARD COMPUTER"
    ...
    # >>>----------------------------------------------------
    # ACTION: sim. SETUP THE SIMULATION ENVIRONMENT
    if [ "$2x" == "simx" ]; then
        # DO NOT ALLOW ADDITIONAL ARGUMENTS FOR THIS ACTION
        LimitNumArgument $0 2 "$@"
        SimBasicSetup
    elif [ "$2x" == "onboardx" ]; then
        # DO NOT ALLOW ADDITIONAL ARGUMENTS FOR THIS ACTION
        LimitNumArgument $0 2 "$@"
        OnboardBasicSetup
    fi

elif [ "$1x" == "gazebo-classic-PILSx" ]; then
    # INPUT STATEMENT 2 VALIDITY CHECK
    # >>>----------------------------------------------------
    declare -A usageState2
    usageState2["run"]="RUN PX4-AUTOPILOT PILS IN GAZEBO-CLASSIC"
    usageState2["debug"]="RUN PX4-AUTOPILOT PILS IN GAZEBO-CLASSIC IN DEBUG MODE (OPTION: SERVICE(S) TO STOP)"
    usageState2["stop"]="STOP PX4-AUTOPILOT PILS IN GAZEBO-CLASSIC IF IT IS RUNNING"
    ...
    # ACTION: debug. RUN THE CONTAINER IN DEBUG MODE (sleep infinity)
        ...
        SetRunModeROS2 $0 gazebo-classic-PILS
    ...
    usageState2["run"]="RUN PX4-AUTOPILOT PILS IN GAZEBO-CLASSIC"
    usageState2["debug"]="RUN PX4-AUTOPILOT PILS IN GAZEBO-CLASSIC IN DEBUG MODE (OPTION: SERVICE(S) TO STOP)"
    usageState2["test"]="UNIT TEST INDIVIDUAL ALGORITHMS IN GAEBZO-CLASSIC-AIRSIM PILS ENVORONMENT"
    usageState2["stop"]="STOP PX4-AUTOPILOT PILS IN GAZEBO-CLASSIC IF IT IS RUNNING"
    ...
    # ACTION: run. RUN THE PX4-GAZEBO-CLASSIC-AIRSIM PILS SIMULATION
        ...
        SetRunModeROS2 $0 gazebo-classic-airsim-PILS
        SetRunModeQGC $0 normal
    # ACTION: test. UNIT TEST INDIVIDUAL ALGORITHMS IN GAZEBO-CLASSIC-AIRSIM PILS ENVIRONMENT
    elif [ "$2x" == "testx" ]; then
    ...
    usageState2["PILS-gazebo-classic-standalone"]="RUN PX4-AUTOPILOT PILS IN GAZEBO-CLASSIC IN STANDALONE MODE (ON ONE CONTAINER)"
    ...
    elif [ "$2x" == "PILS-gazebo-classic-standalonex" ]; then
        # DO NOT ALLOW ADDITIONAL ARGUMENTS FOR THIS ACTION
        LimitNumArgument $0 2 "$@"
        SetRunModePX4 $0 PILS-gazebo-classic-standalone
    fi
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    # >>>----------------------------------------------------
    declare -A usageState2
    usageState2["PILS-px4"]="RUN PX4-AUTOPILOT PILS IN GAZEBO-CLASSIC"
